...
def main(
    ...
    # history = simulate_gol(init_state=state, num_steps=num_steps)
...
def simulate_gol(
    ...
    def step(prev_state, _input):
        neighbours = (
            ...
            + prev_state[0:-2, 2:  ]
            + prev_state[1:-1, 0:-2]
            # don't add center: prev_state[1:-1, 1:-1]
            + prev_state[1:-1, 2:  ]
            + prev_state[2:  , 0:-2]
            + prev_state[2:  , 1:-1]
            + prev_state[2:  , 2:  ]
        ...
        next_state = (neighbours == 3) | ((neighbours == 2) & (prev_state[1:-1,1:-1] == 1))
        next_state = next_state.astype(np.uint8)
        next_state = jnp.pad(next_state, 1, mode='wrap')
        return next_state, next_state
    
    # for _ in range(num_steps):
    #     next_state, _ = step(history[-1], history[-1])
